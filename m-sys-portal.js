...
})(typeof self !== 'undefined' ? self : this, function(__WEBPACK_EXTERNAL_MODULE_66__, __WEBPACK_EXTERNAL_MODULE_42__, __WEBPACK_EXTERNAL_MODULE_22__, __WEBPACK_EXTERNAL_MODULE_59__, __WEBPACK_EXTERNAL_MODULE_138__) {
...
var _options = {
	...
	webapiMap: {
		'portal.init': '/v1/web/portal/init',
...
function getMeta() {
	return {
		name: 'root',
		component: '::div',
		className: 'm-sys-portal',
		children: [{
			name: 'tabBar',
			component: '::div',
			style: { height: '100%' },
			className: '{{(data.content && data.content.appName) ? \'m-sys-portal-hidden\': \'m-sys-portal-show\'}}',
			children: {
				name: 'tabBar',
				component: 'TabBar',
				...
				children: [{
					...
					selected: '{{$tabIsSelected(data, \'acc\')}}',
					onPress: '{{()=>$tabChange(\'acc\')}}',
					icon: {
						name: 'i',
						component: 'RRIcon',
						icon: 'liushui-moren'
					...
					selectedIcon: {
						name: 'i',
						component: 'RRIcon',
						icon: 'liushui-xuanzhong'
					},
					children: [getNavBar(), {
						name: 'rpt',
						component: 'AppLoader',
						currOrgId: '{{data.currOrgId}}',
						appName: 'm-acc-main'
					}]
				}, {
					name: 'rpt',
					...
					key: 'rpt',
					component: 'TabBar.Item',
					selected: '{{$tabIsSelected(data, \'rpt\')}}',
					onPress: '{{()=>$tabChange(\'rpt\')}}',
					icon: {
						name: 'i',
						component: 'RRIcon',
						icon: 'baobiao-moren'
					...
					key: 'inv',
					component: 'TabBar.Item',
					selected: '{{$tabIsSelected(data, \'inv\')}}',
					onPress: '{{()=>$tabChange(\'inv\')}}',
					icon: {
						name: 'i',
						component: 'RRIcon',
						icon: 'fapiao-moren'
			...
			component: '::div',
			style: { height: '100%' },
			className: '{{(data.content && data.content.appName) ? \'m-sys-portal-openPages m-sys-portal-show\': \'m-sys-portal-openPages m-sys-portal-hidden\'}}',
			children: [{
				name: 'app',
				component: 'AppLoader',
				appName: '{{ data.openPages && data.openPages.length > 0 && data.openPages[_rowIndex].appName }}',
				'...': '{{data.openPages && data.openPages.length > 0 && data.openPages[_rowIndex].appProps}}',
				_power: 'for in data.openPages'
			}]
		}, {
			name: 'leftDrawer',
			component: 'Drawer',
			className: 'm-sys-portal-left-drawer',
			enableDragHandle: true,
			_visible: '{{data.showLeftDrawer}}',
			sidebar: {
				name: 'sidebar',
				component: '::div',
				children: {
					...
					showLeftDrawer: '{{data.showLeftDrawer}}'
				}
			},
			open: '{{data.showLeftDrawer}}',
			onOpenChange: '{{$openLeft}}'
		}]
	};
}
...
function getNavBar() {
	return {
		...
		component: 'NavBar',
		mode: 'light',
		onLeftClick: '{{$openLeft}}',
		icon: {
			name: 'left',
			component: 'RRIcon',
			icon: 'liebiao',
			className: '{{data.showLeftDrawer ? \'m-sys-portal-left-drawer-show\': \'m-sys-portal-left-drawer-hidden\'}}'
		},
		children: '{{$getCurrentOrgName(data)}}'
	};
}
...
var _mkComponent = __webpack_require__(138);
...
var action = function action(option) {
    ...
    this.onInit = function (_ref) {
        ...
        var setStatusBarBg = _this.component.props.setStatusBarBg,
            setStatusBottomCorol = _this.component.props.setStatusBottomCorol;
        injections.reduce('init');
        _this.metaAction.context.set('openPage', _this.setContent);
        setStatusBarBg && setStatusBarBg('#2FAAF9');
        setStatusBottomCorol && setStatusBottomCorol('#F8FCFF');
        _this.load();
        //history增加
        _mkUtils.history.listen('m-sys-portal', _this.listen);
    };

    this.componentWillUnmount = function () {
        _mkUtils.history.unlisten('m-sys-portal', _this.listen);
    };

    this.listen = function (childApp, location, action) {

        var currentAppName = _this.metaAction.gf('data.content.appName');
        var targetAppName = childApp;
        //if (!targetAppName && action == 'POP') {
        if (action == 'POP') {

            _this.injections.reduce('closeContent', currentAppName);
            return;
        }
        ...
        _this.setContent('', targetAppName);
    };

    this.load = function () {
        var _ref2 = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee() {
            var orgId = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : -1;
            var response;
                    ...
                    switch (_context.prev = _context.next) {
                        case 0:
                            if (window.sessionStorage.orgId) {
                                orgId = window.sessionStorage.orgId;
                            }

                            _context.next = 3;
                            return _this.webapi.portal.init({
                                orgId: orgId, dzOrgId: "", isServiceProvider: false
                            });

                        case 3:
                            response = _context.sent;

                            window.sessionStorage.setItem('orgId', '');
                            if (response) {
                                _this.metaAction.context.set('currentUser', response);
                                _this.metaAction.context.set('appInfo', response.app);
                                _this.metaAction.context.set('userInfo', response.user);
                                _this.metaAction.context.set('currOrg', response.currOrg);
                                _this.metaAction.context.set('orgList', response.orgs);
                                _this.metaAction.sfs({
                                    'data.other.currentUser': (0, _immutable.fromJS)(response),
                                    'data.showLeftDrawer': false,
                                    'data.currOrgId': response.currOrgId,
                                    'data.selectedTab': 'rpt'
                                });
                            } else {
    ...
    this.getCurrentOrgName = function (data) {
        var currOrg = _this.metaAction.context.get('currOrg');
        if (!currOrg) return;
        return currOrg.name;
    };

    this.tabChange = function (key) {
        _this.metaAction.sf('data.selectedTab', key);
    };

    this.tabIsSelected = function (data, key) {
        return data.selectedTab == key;
    };

    this.isActivePage = function (data, rowIndex) {
        if (!data.content || !data.openPages || data.openPages.length == 0) return false;

        var appName = data.openPages && data.openPages.length > 0 && data.openPages[rowIndex].appName;
        return data.content.appName == appName;
    };

    this.openLeft = function () {
        _this.metaAction.sf('data.showLeftDrawer', !_this.metaAction.gf('data.showLeftDrawer'));
    };
    ...
    this.setContent = function (name, appName) {
        var appProps = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};

        return new _promise2.default(function (resolve, reject) {
            appProps = (0, _extends3.default)({}, appProps, { onOk: function onOk(result) {
                    resolve(result);
                } });
            _this.injections.reduce('setContent', name, appName, appProps);
        });
    };
...
var reducer = function reducer(option) {
    ...
    this.init = function (state, option) {
        ...
        return state;
        var childApp = _mkUtils.history.getChildApp('m-sys-portal');
        if (childApp) return _this.setContent(state, '', childApp);else return _this.setContent(state, '首页', 'm-sys-home', { isEmbed: true });
    };

    this.setContent = function (state, name, appName, appProps) {
        //判断当前显示页签appName和要新打开的是否一致
        var currContent = _this.metaReducer.gf(state, 'data.content');
        if (currContent && appName == currContent.get('appName')) return state;

        //history增加
        var menuAppNameMap = _this.metaReducer.gf(state, 'data.menuAppNameMap');
        if (name && appName && menuAppNameMap.getIn([appName, 'name']) != name) {
            menuAppNameMap = menuAppNameMap.set(appName, (0, _immutable.fromJS)({ name: name, props: appProps }));
            state = _this.metaReducer.sf(state, 'data.menuAppNameMap', menuAppNameMap);
        }

        name = menuAppNameMap.getIn([appName, 'name']);
        appProps = appProps || menuAppNameMap.getIn([appName, 'props']);

        var content = (0, _immutable.fromJS)({ name: name, appName: appName, appProps: appProps });
        state = _this.metaReducer.sf(state, 'data.content', content);

        var openPages = _this.metaReducer.gf(state, 'data.openPages') || (0, _immutable.List)();
        var hitIndex = openPages.findIndex(function (o) {
            return o.get('name') == name || o.get('appName') == appName;
        });
        var hit = hitIndex != -1;
        var isTabsStyle = _this.metaReducer.gf(state, 'data.isTabsStyle');

        if (!hit) {
            openPages = openPages.push(content);
            state = _this.metaReducer.sf(state, 'data.openPages', openPages);
        } else {
            state = _this.metaReducer.sf(state, 'data.openPages.' + hitIndex, content);
        }

        // if(!appProps.isEmbed){
        setTimeout(function () {
            _mkUtils.history.pushChildApp('m-sys-portal', content.get('appName'));
        }, 0);
        // }
    ...
    this.closeContent = function (state, name) {
        var openPages = _this.metaReducer.gf(state, 'data.openPages') || (0, _immutable.List)();
        var hitIndex = openPages.findIndex(function (o) {
            return o.get('name') == name;
        });
        openPages = openPages.remove(hitIndex);
        state = _this.metaReducer.sf(state, 'data.openPages', openPages);
        return _this.metaReducer.sf(state, 'data.content', openPages.get(openPages.size - 1));
    };

    this.closeAll = function (state) {
        state = _this.metaReducer.sf(state, 'data.openPages', new _immutable.List());
        return _this.metaReducer.sf(state, 'data.content', new _immutable.Map());
    };
...
exports.default = {
    name: "m-sys-portal",
    version: "1.0.0",
    description: "m-sys-portal",
    meta: data.getMeta(),
    components: [],
    config: config,
    load: function load(cb) {
        cb(__webpack_require__(54), __webpack_require__(60), __webpack_require__(67));
    }
};
module.exports = exports['default'];
...
exports.default = {
    portal: {
        init: function init(option) {
            return _mkUtils.fetch.post(api('portal.init'), option);
        }
    },
    user: {
        logout: function logout() {
            return _mkUtils.fetch.post(api('user.logout'));
        }
    }
};
module.exports = exports['default'];
