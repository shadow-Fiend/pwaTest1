...
function getMeta() {
	return {
		...
		children: [{
			...
			component: 'List.Item',
			arrow: 'down',
			className: 'm-inv-check-invType',
			children: '发票类型',
			extra: '{{data.form.bizType ? data.form.bizType :"请选择发票类型"}}',
			onClick: '{{$listItemClick(\'bizType\')}}'
		}, {
			name: 'invoiceCode',
			component: 'InputItem',
			className: 'm-inv-check-content',
			children: '发票代码',
			placeholder: '请输入10位或12位发票代码',
			maxLength: '12',
			onChange: "{{(v)=>$sf('data.form.invoiceCode', v)}}",
			...
			onChange: "{{(v)=>$sf('data.form.invoiceNumber', v)}}",
			value: '{{data.form.invoiceNumber}}'
		}, {
			name: 'amountWithoutTax',
			component: 'InputItem',
			_visible: '{{data.TaxDisplay}}',
			children: '不含税金额',
			placeholder: '请输入不含税金额',
			onChange: "{{ (v)=>$sf('data.form.amountWithoutTax',v)}}",
			value: '{{data.form.amountWithoutTax}}'
		}, {
			name: 'invoiceParityCode',
			component: 'InputItem',
			_visible: '{{data.CodeDisplay}}',
			children: '校验码',
			maxLength: '6',
			placeholder: '请输入6位校验码',
			onChange: "{{ (v)=>$sf('data.form.invoiceParityCode',v)}}",
			value: '{{data.form.invoiceParityCode}}'
		}, {
			name: 'date',
			component: 'DatePicker',
			mode: 'date',
			extra: '{{data.form.invoiceDate ? data.form.invoiceDate :"请选择开票日期"}}',
			onChange: '{{$dateChange}}',
			children: {
				name: 'dateList',
				component: 'List.Item',
			...
			name: 'btn',
			component: '::div',
			className: 'm-inv-check-btn',
			_visible: '{{data.showBtn}}',
			children: [{
				name: 'checking',
				component: 'Button',
				className: 'm-inv-check-checking',
				children: '查验',
				onClick: '{{$invoiceCheck}}',
				disabled: '{{$getCheckDis()}}'
			}, {
				name: 'replacement',
				component: 'Button',
				className: 'm-inv-check-replacement',
				children: '重置',
				onClick: '{{$replacement}}'
			}]
		}, {
			...
			className: 'm-inv-check-bgPhoto',
			children: [{
				name: 'bgPhotoContent',
				component: '::div',
				className: 'm-inv-check-loadingPhoto'
			}]
		}, {
			name: 'bizTypeModal',
			component: 'Modal',
			...
			children: {
				...
				children: {
					...
					key: '{{data.other.bizTypes[_rowIndex].value}}',
					onChange: '{{()=>$invTypeChange(data)(data.other.bizTypes[_rowIndex].value)}}',
					children: '{{data.other.bizTypes[_rowIndex].label}}',
...
function getInitState() {
	return {
		data: {
			showBizTypeModal: false, //业务类型
			CodeDisplay: false, //校验码
			TaxDisplay: true, //不含税金额
			showBtn: true,
...
var action = function action(option) {
  ...
  this.onInit = function (_ref) {
    ...
    var checkData = _this.component.props.data;
    if (checkData) {
      _this.load(checkData);
    }
  };

  this.load = function (checkData) {
    debugger;
    if (checkData && checkData.text) {
      var nowData = checkData.text.split(",");

      if (nowData[0] == '01') {
        var _nowData = checkData.text.split(","),
            nowDateYear = _nowData[5].substring(0, 4),
            nowDateMon = _nowData[5].substring(4, 6),
            nowDateDay = _nowData[5].substring(6, 8),
            nowDate = nowDateYear + '-' + nowDateMon + '-' + nowDateDay;
        if (_nowData[1] == '04') {
          _this.metaAction.sfs({
            'data.form.bizType': '增值税普通发票',
            'data.form.invoiceParityCode': _nowData[6].substring(14, 20),
            'data.form.invoiceType': '04',
            'data.CodeDisplay': true,
            'data.TaxDisplay': false
          });
        } else if (_nowData[1] == '01') {
          _this.metaAction.sfs({
            'data.form.bizType': '增值税专用发票',
            'data.form.amountWithoutTax': _nowData[4],
            'data.form.invoiceType': '01',
            'data.CodeDisplay': false,
            'data.TaxDisplay': true
          });
        } else {
          _this.metaAction.toast('info', ' 暂不支持该类型发票');
          _this.metaAction.sfs({
            'data.form.invoiceCode': '',
            'data.form.invoiceNumber': '',
            'data.form.invoiceDate': ''
          });
        }

        _this.metaAction.sfs({
          'data.form.invoiceCode': _nowData[2],
          'data.form.invoiceNumber': _nowData[3],
          'data.form.invoiceDate': nowDate
        });
      }
    } else if (checkData && checkData == 'checkData') {
      _this.metaAction.toast('info', '扫描失败请手动输入');
    }
  };
  ...
  this.invoiceCheck = function () {
    var nowData = _this.metaAction.gf('data.form').toJS();

    if (nowData.invoiceCode.match(/^[0-9]{10}$/) || nowData.invoiceCode.match(/^[0-9]{12}$/)) {
      if (nowData.invoiceNumber.match(/^[0-9]{8}$/)) {
        if (nowData.invoiceType == "04") {
          if (nowData.invoiceParityCode.match(/^[0-9]{6}$/)) {
            _this.check();
          } else {
            _this.metaAction.toast('info', '请输入6位校验码');
          }
        } else {
          _this.check();
        }
      } else {
        _this.metaAction.toast('info', '请输入8位发票号码');
      }
    } else {
      _this.metaAction.toast('info', '请输入10位或12位发票代码');
    }
  };

  this.check = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee() {
    var nowData, response;
    return _regenerator2.default.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _this.metaAction.sfs({
              'data.showBgPhoto': true
            });
            nowData = _this.metaAction.gf('data.form').toJS();
            _context.next = 4;
            return _this.webapi.check(nowData);
          ...
          case 4:
            ...
            response.result && _this.openResult(response);
      ...
      return _regenerator2.default.wrap(function _callee2$(_context2) {
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              // this.metaAction.sf('data.showBtn',false)
              ...
              _context2.next = 4;
              return openPage('查验结果', 'm-inv-checkResult', { data: data });
            ...
            case 4:
              ...
              if (result.a == '1') {
                _this.metaAction.sf('data.showBtn', true);
              } else if (result.a == '2') {
                _this.metaAction.sfs({
                  'data.showBtn': true,
                  'data.form.bizType': '增值税专用发票',
  ...
  this.replacement = function () {
    _this.metaAction.sfs({
      'data.form.invoiceCode': "", //发票代码
      'data.form.invoiceNumber': "", //发票号码
      'data.form.invoiceDate': '', //开票日期
      'data.form.invoiceType': '', //发票类型
      'data.form.amountWithoutTax': '', //不含税金额
      'data.form.invoiceParityCode': '', //校验码
      'data.form.bizType': '',
      'data.CodeDisplay': false,
      'data.TaxDisplay': true
    });
  ...
  this.listItemClick = function (type) {
    return function () {
      _this.metaAction.sf('data.showBizTypeModal', true);
    };
  };

  this.getCheckDis = function (data) {
    var info = _this.metaAction.gf('data.form');

    if (info && info.get('invoiceType') == '01') {
      if (info && info.get('invoiceCode') && info.get('invoiceNumber') && info.get('invoiceDate') && info.get('invoiceType') && info.get('amountWithoutTax')) {
        return false;
      }
    } else {
      if (info && info.get('invoiceCode') && info.get('invoiceNumber') && info.get('invoiceDate') && info.get('invoiceType') && info.get('invoiceParityCode')) {
        return false;
      }
    }
  ...
  this.dateChange = function (v) {
    _this.metaAction.sfs({
      'data.form.invoiceDate': (0, _moment2.default)(v).format('YYYY-MM-DD')
    });
  };

  this.invTypeChange = function (data) {
    return function (v) {
      var bizType = data.other.bizTypes.find(function (o) {
        return o.value == v;
      });

      _this.metaAction.sfs({
        'data.showBizTypeModal': false,
        'data.form.bizType': bizType.label,
        'data.form.invoiceType': bizType.value
      });

      var nowInvoiceType = _this.metaAction.gf('data.form.invoiceType');
      if (nowInvoiceType == '04') {
        _this.metaAction.sfs({
          'data.CodeDisplay': true,
          'data.TaxDisplay': false
        });
...
var reducer = function reducer(option) {
    ...
    this.loadInfo = function (state, response) {
        var sss = response;
        return state;
    };
...
var api = function api(key) {
    var url = _config2.default.current.webapiMap['check'],
...
exports.default = {
    check: function check(option) {
        return _mkUtils.fetch.post(api('check'), option);
    }
};
module.exports = exports['default'];
