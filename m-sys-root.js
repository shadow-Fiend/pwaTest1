(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["MSysRoot"] = factory(require("immutable"), require("MK")["metaEngine"], require("MK.utils"), require("react"));
	else
		root["MSysRoot"] = factory(root["Immutable"], root["MK.metaEngine"], root["MK"]["utils"], root["React"]);
})(typeof self !== 'undefined' ? self : this, function(__WEBPACK_EXTERNAL_MODULE_128__, __WEBPACK_EXTERNAL_MODULE_39__, __WEBPACK_EXTERNAL_MODULE_33__, __WEBPACK_EXTERNAL_MODULE_57__) {
...
var _webapi = __webpack_require__(72);
...
function getMeta() {
    return {
        ...
        children: {
            ...
            setStatusBottomCorol: '{{$setStatusBottomCorol}}'
        }
    };
}
...
function getInitState() {
    return {
        data: {
            currentAppName: _config2.default.current.startAppName
        }
    };
}
...
var action = function action(option) {
    ...
    this.onInit = function () {
        var _ref = (0, _asyncToGenerator3.default)( /*#__PURE__*/_regenerator2.default.mark(function _callee(_ref2) {
            ...
            var initState, defaultAppName, currentAppName, response;
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        case 0:
                            ...
                            initState = (0, _data.getInitState)(), defaultAppName = initState.data.currentAppName, currentAppName = _mkUtils.history.getChildApp('m-sys-root') || defaultAppName;


                            initState.data.currentAppName = currentAppName;
                            injections.reduce('init', { initState: initState });

                            _mkUtils.history.listen('m-sys-root', _this.listen);

                            if (!(localStorage.getItem('remember') && localStorage.getItem('password') && localStorage.getItem('account'))) {
                                _context.next = 11;
                                break;
                            }
                        ...
                        case 9:
                            ...
                            if (response) {
                                _this.metaAction.context.set('currentUser', response.sysUser);
                                defaultAppName = 'm-sys-portal';
                            } else {
                                localStorage.setItem('password', '');
                            }

                        case 11:
                            _this.onRedirect({ appName: defaultAppName });
    ...
    this.listen = function (childApp, location, action) {
        ...
        if (action == 'POP' && targetAppName == 'm-sys-login' && (currentAppName == 'm-sys-portal' || currentAppName == 'm-sys-login')) {
            if (_this.backClick > 0) {
                _this.backClick = 0;
                window.navigator && window.navigator.app.exitApp();
            } else {
                _this.metaAction.toast('info', '再按一次退出');
                if (new Date() - _this.time < 500) {
                    _this.backClick++;
                } else {
                    _this.time = new Date();
                }
    ...
    this.onRedirect = function (_ref3) {
        var appName = _ref3.appName,
            appParams = _ref3.appParams;

        _mkUtils.history.pushChildApp('m-sys-root', appName, (0, _extends3.default)({}, appParams));
    };

    this.setStatusBarBg = function (color) {
        if (!color || !window.statusBar) return;
        if (navigator.platform == "iPhone") {
            document.getElementById('app').style.borderTopColor = color;
        } else {
            window.statusBar.backgroundColorByHexString(color);
        }
    };

    this.setStatusBottomCorol = function (color) {
        if (!color || !window.statusBar) return;
        if (navigator.platform == "iPhone") {
            document.getElementById('app').style.borderBottomColor = color;
        }
    };

    this.statusBarVisible = function (visible) {
        if (!visible && window.statusBar) return window.statusBar.isVisible;
        if (window.statusBar && visible == 'hide') return window.statusBar.hide();
        if (window.statusBar && visible == 'show') return window.statusBar.show();
    };

    this.componentWillUnmount = function () {
        _mkUtils.history.unlisten('m-sys-root', _this.listen);
    };
...
__webpack_require__.p = window["__pub_root__"];
...
exports.default = {
    name: "m-sys-root",
    version: "1.0.0",
    description: "m-sys-root",
    meta: data.getMeta(),
    components: [],
    config: config,
    load: function load(cb) {
        cb(__webpack_require__(46), __webpack_require__(58), __webpack_require__(64));
    }
};
module.exports = exports['default'];
